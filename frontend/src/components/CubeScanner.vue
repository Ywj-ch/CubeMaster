        ...
        <div class="camera-section">
          <div class="camera-wrapper">
            ...
            <div class="instruction-overlay">
              <h3 class="step-title">步骤 {{ currentFaceIndex + 1 }}/6: 扫描{{ scanSteps[currentFace].title }}</h3>
              <p class="step-guide">
                 <el-icon><Aim /></el-icon>
        ...
        <div class="scanner-footer">
          <div class="preview-strip">
            <div
              ...
              :key="face"
              class="preview-slot"
              :class="{ active: index === currentFaceIndex, completed: scannedImages[face] }"
            >
              <img v-if="scannedImages[face]" :src="scannedImages[face]" class="slot-img" />
              <span v-else class="slot-label">{{ face }}面</span>
            </div>
          </div>

          <div class="action-area">
             <el-button class="reset-btn" link @click="resetScanner" :disabled="currentFaceIndex === 0">
               重拍
             </el-button>

             <button class="shutter-button" @click="capture" :disabled="isProcessing">
               <div class="inner-circle" :class="{ 'is-loading': isProcessing }"></div>
             </button>

             <div class="placeholder-spacer"></div> </div>
        </div>
      </div>
    </div>
  </transition>
...
.close-icon-btn {
  background: none; border: none; cursor: pointer; padding: 4px; color: #86868b; transition: color 0.2s;
}
...
.v-1 { left: 33.3%; top: 0; bottom: 0; width: 1px; }
.v-2 { left: 66.6%; top: 0; bottom: 0; width: 1px; }
.h-1 { top: 33.3%; left: 0; right: 0; height: 1px; }
.h-2 { top: 66.6%; left: 0; right: 0; height: 1px; }
.corner { position: absolute; width: 24px; height: 24px; border: 3px solid #0071e3; /* 加粗角标 */ }
.tl { top: -2px; left: -2px; border-right: none; border-bottom: none; border-radius: 8px 0 0 0; }
.tr { top: -2px; right: -2px; border-left: none; border-bottom: none; border-radius: 0 8px 0 0; }
.bl { bottom: -2px; left: -2px; border-right: none; border-top: none; border-radius: 0 0 0 8px; }
.br { bottom: -2px; right: -2px; border-left: none; border-top: none; border-radius: 0 0 8px 0; }
...
.inner-circle {
  width: 56px; height: 56px; border-radius: 50%; background: linear-gradient(145deg, #ffffff, #f0f0f0); border: 1px solid rgba(0,0,0,0.05);
}
